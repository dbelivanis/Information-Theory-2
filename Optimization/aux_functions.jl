...
using Plots
    ...
    function update_K_p(sess,model_param,tf_variables,check_diff,N_k_dis_,p_pre_soft_max)
        ...
        if check_diff > 5e-5 && N_k_dis_ < N_k
            ...
            k_x_t_update = k_x_t_update .+ (0.0 .+ 1e-0 .* (0.5 .- rand(N_k,model_param.N_steps
        )))
                    ...
                    k_y_t_update = k_y_t_update .+ (0.0 .+ 1e-0.* (0.5 .- rand(N_k,model_param.N_steps
        )))
                    ...
                    k_xy_t_update = k_xy_t_update .+ (0.0 .+ 5e-2 .* (0.5 .- rand(N_k,model_param.N_steps
        )))
                    run(sess,tf.assign(k_x_t_log,k_x_t_update));
                    ...
                    k_x_t_update = run(sess,k_x_t_log) .+ (0.0 .+1e-0 *  (rand(N_k,model_param.N_steps
        ).-0.5));
                    run(sess,tf.assign(k_x_t_log,k_x_t_update));
                        
                    k_y_t_update = run(sess,k_y_t_log) .+ (0.0 .+1e-0 *  (rand(N_k,model_param.N_steps
        ).-0.5));
                    run(sess,tf.assign(k_y_t_log,k_y_t_update));
